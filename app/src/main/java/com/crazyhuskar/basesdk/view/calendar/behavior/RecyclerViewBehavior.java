...
public class RecyclerViewBehavior extends CoordinatorLayout.Behavior<RecyclerView> {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, RecyclerView child, int layoutDirection) {
        ...
        child.offsetTopAndBottom(CalendarUtil.loadTop());
    ...
    public boolean onStartNestedScroll(CoordinatorLayout coordinatorLayout, RecyclerView child,
        ...
        return isVertical && (recycleviewTopStatus || !CalendarUtil.isScrollToBottom()) && child == directTargetChild;
    }
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, RecyclerView child,
        ...
        if (child.getTop() <= initOffset && child.getTop() >= minOffset) {
            consumed[1] = CalendarUtil.scroll(child, dy, minOffset, initOffset);
    ...
    public void onStopNestedScroll(final CoordinatorLayout parent, final RecyclerView child, View target) {
        ...
        if (!CalendarUtil.isScrollToBottom()) {
            if (initOffset - CalendarUtil.loadTop() > CalendarUtil.getTouchSlop(context)){
                scrollTo(parent, child, minOffset, 200);
            } else {
                scrollTo(parent, child, initOffset, 80);
            }
        } else {
            if (CalendarUtil.loadTop() - minOffset > CalendarUtil.getTouchSlop(context)){
    ...
    private void scrollTo(final CoordinatorLayout parent, final RecyclerView child, final int y, int duration){
        ...
        scroller.startScroll(0, CalendarUtil.loadTop(), 0, y - CalendarUtil.loadTop(), duration);   //设置scroller的滚动偏移量
    ...
    private void saveTop(int top){
        CalendarUtil.saveTop(top);
        if (CalendarUtil.loadTop() == initOffset){
            CalendarUtil.setScrollToBottom(false);
        } else if (CalendarUtil.loadTop() == minOffset){
